class Survey {
    ...
    constructor(customSurveyDetails = {}) {
      ...
      this.globalStyles = this.mergeStyles(
        Survey.defaultStyles,
        this.surveyDetails.styles || {}
      );
    ...
    mergeStyles(defaultStyles, customStyles) {
      return Object.fromEntries(
        Survey.styleKeys.map((key) => [
          key,
          this.deepMerge(defaultStyles[key], customStyles[key]),
        ])
      );
    }
    ...
    generateStylesheet() {
      return Survey.styleKeys
        .map((key) =>
          this.generateStyleForSelector(
            this.getSelectorForKey(key),
            this.globalStyles[key]
          )
        )
        .join("\n");
    }
      ...
      Object.entries(rules)
        ...
        .forEach(([key, value]) => {
          if (key.startsWith("@media")) {
            styleString += `\n${key} { ${selector} { ${this.rulesToString(
              value
            )} } }`;
          } else if (key.startsWith("&")) {
            styleString += `\n${selector}${key.slice(1)} { ${this.rulesToString(
              value
            )} }`;
          }
        });
    ...
    validateStyles() {
      Object.keys(this.globalStyles).forEach((key) => {
        if (!Survey.styleKeys.includes(key)) {
          console.warn(
            `Invalid style key '${key}' for Survey. Valid keys are: ${Survey.styleKeys.join(
              ", "
            )}`
          );
        }
    ...
    responded(questionId) {
      const responses = this.responses.filter((r) => r.id === questionId);
      return responses.length > 0
        ? responses[responses.length - 1].responded
        : false;
    }
    ...
    finishSurvey(message) {
      try {
        ...
        const finishElement =
          document.getElementById("finish") || document.createElement("div");
